...
public class DataCleanManager {
    ...
    /**方法deleteFilesByDirectory()无效时，则使用该方法*/
	/*private static boolean deleteFilesByDirectory(File dir, boolean deleteThisFile) {
		if(dir != null && dir.isDirectory()) {
			String[] children = dir.list();
			for(int i = 0; i < children.length; i++) {
				boolean success = deleteFilesByDirectory(new File(dir, children[i]), true);
				if(!success) return false;
			}
		}
		if(dir != null) {
			if(deleteThisFile) return dir.delete();
			else return true;
		}
		return false;
	}*/
